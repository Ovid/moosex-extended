...
For most sane codebases, converting from L<Moose> to L<MooseX::SafeDefaults> is as
simple as C<< s/Moose/MooseX::SafeDefaults/ >>. We I<try> to be
...
For roles, just C<< s/Moose::Role/MooseX::SafeDefaults::Roles/ >> and repeat the
...
L<MooseX::SafeDefaults> is built on years of experience hacking on L<Moose> and
...
We get tired of typing a lot of boilerplate, so C<MooseX::SafeDefaults> does away
with it.
...
this is simply forget about it. C<MooseX::SafeDefaults> solves with by separating
...
C<MooseX::SafeDefaults> applies C<MooseX::StrictConstructor> to your class so you
never have to face this problem again.

B<WARNING>: Be careful when using this in existing class hierarchies. While
C<MooseX::SafeDefaults> is compatible with Moose (Moose classes can always use
C<MooseX::SafeDefaults> classes and roles), the reverse isn't always true. We've
...
L<MooseX::SafeDefaults> does it for you.
...
classes if they use C<MooseX::SafeDefaults>:
