...
When using C<field> or C<param>, we have some attribute shortcuts:
    ...
    param name => (
        ...
        initializer => 1,             # _initialize_name
    );
...
If an attribute has C<writer> is set to C<1> (the number one), it will refer
to a method named C<set_$attribute_name> is created.
...
If an attribute has C<reader> is set to C<1> (the number one), it will refer
to a method named C<get_$attribute_name> is created.
...
If an attribute has C<predicate> is set to C<1> (the number one), it will
refer to a method named C<has_$attribute_name> is created.
...
If an attribute has C<clearer> is set to C<1> (the number one), it will refer
to a method named C<clear_$attribute_name> is created.
...
If an attribute has C<builder> is set to C<1> (the number one), it will refer
to a method named C<_build_$attribute_name>.
...
If an attribute has C<initializer> is set to C<1> (the number one), it will
refer to a method named C<_initialize_$attribute_name>.
...
Is the same as this:
    ...
    has title => (
        is          => 'ro',
        isa         => NonEmptyStr,
        initializer => '_iinitialize_title',
    );
